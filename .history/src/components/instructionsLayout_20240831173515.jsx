...
export default function InstructionsLayout({ title, steps }) {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full md:w-1/4 transform transition duration-500 hover:scale-105 mb-8">
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        {steps.map(({ step, title, description }, index) => {
                            const colors = ['blue', 'green', 'yellow', 'red']; // צבעים קבועים
                            const color = colors[index % colors.length]; // מעגל את הצבעים לפי האינדקס
                            return (
