...
export const PeerIdProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        peer.on("connection", (con) => {
            console.log("Incoming connection from peer:", con.peer);
            openConfirmModal(
                "Incoming Connection",
                ...
                if (confirmed) {
                    con.on("open", () => {
                        console.log("Connection accepted with peer:", con.peer);
    ...
    const connectRecipient = (peerIdToConnect) => {
        if (connection) {
            disconnect();
        } else {
            const con = peer.connect(peerIdToConnect);
            con.on("open", () => {
                console.log("Outgoing connection to:", peerIdToConnect);
                setRecipient(peerIdToConnect);
                setRecipientPeerId(peerIdToConnect);
                setConnection(con);
            });
