...
export const PeerIdProvider = ({ children }) => {
            ...
            openConfirmModal("Incoming Connection", `Do you want to accept the connection request from ${con.peer}?`)
                .then((accepted) => {
                    if (accepted) {
                        ...
                        // חיווי שווידוא שהחיבור דו-כיווני נעשה
                        con.send(JSON.stringify({ messageType: "connect_ack" }));
                    } else {
                        con.close();
                    }
                });
    ...
    const handleData = (data) => {
        try {
            ...
            if (parsedData.messageType === "connect_ack") {
                console.log("Connection is now fully established");
                return;
            }
