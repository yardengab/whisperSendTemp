...
export const PeerIdProvider = ({ children }) => {
            ...
            openConfirmModal("Incoming Connection", `Do you want to accept the connection request from ${con.peer}?`)
                .then((accepted) => {
                    if (accepted) {
                        ...
                        con.on("close", () => {
                            handleRemoteDisconnect(); // פונקציה חדשה לטיפול בהתנתקות מהצד השני
                        });
                    } else {
                        con.close();
                    }
    ...
    const disconnect = () => {
        if (connection) {
            openConfirmModal("Confirm Disconnect", "Are you sure you want to disconnect?")
                .then((confirmed) => {
                    if (confirmed) {
                        connection.send(JSON.stringify({ type: "disconnect" })); // שולחים הודעה למשתמש השני
                        connection.close();
                        setConnection(null);
        ...
        openConfirmModal("Remote Disconnect", "The other user has disconnected. Do you want to disconnect as well?")
            .then((confirmed) => {
                if (confirmed) {
                    ...
                    setIsDisconnected(true); // משתמש מנותק, אבל נשאר בצ'אט
                }
            });
    };
    ...
    const handleData = (data, senderPeerId) => {
        try {
            ...
            if (parsedData.type === "disconnect") {
                handleRemoteDisconnect(); // אם מתקבלת הודעת ניתוק, הפונקציה מופעלת
                return;
            }
    ...
    return (
        <PeerIdContext.Provider
            value={{
                ...
                setRecipientPeerId,
                openConfirmModal,
                isDisconnected, // מעבירים את המשתנה כדי לעדכן את מצב ההתנתקות
            }}
        >
            {children}
            {showConfirmModal && (
