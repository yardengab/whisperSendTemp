...
export const PeerIdProvider = ({ children }) => {
    ...
    const [isDisconnected, setIsDisconnected] = useState(false); // משתנה חדש למעקב אחרי מצב החיבור
            ...
            openConfirmModal("Incoming Connection", `Do you want to accept the connection request from ${con.peer}?`)
                .then((accepted) => {
                    if (accepted) {
                        ...
                        con.on("close", () => {
                            handleRemoteDisconnect(); // קריאה לפונקציה חדשה
                        });
                    } else {
                        con.close();
                    }
    ...
    const disconnect = () => {
        openConfirmModal("Disconnect", "Are you sure you want to disconnect?").then((confirmed) => {
            if (confirmed) {
                ...
                setIsDisconnected(true); // עדכון המצב לאחר ניתוק
                setConnection(null);
                setRecipient("");
                setRecipientPeerId("");
                setMessages([]);
    ...
    const handleRemoteDisconnect = () => {
        openConfirmModal("Remote Disconnect", "The other user has disconnected. Do you want to disconnect as well?").then((confirmed) => {
            if (confirmed) {
                disconnect(); // ניתוק והפנייה ל-sharesecurly אם המשתמש השני רוצה להתנתק
            } else {
                setIsDisconnected(true); // עדכון המצב לאחר ניתוק
            }
        });
    ...
    return (
        <PeerIdContext.Provider
            value={{
                ...
                setRecipientPeerId,
                openConfirmModal,
                isDisconnected, // הוספת המשתנה להקשר
            }}
        >
            {children}
            {showConfirmModal && (
