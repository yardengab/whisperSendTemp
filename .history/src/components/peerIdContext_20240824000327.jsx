...
export const PeerIdProvider = ({ children }) => {
    ...
    const [isInitiatingDisconnect, setIsInitiatingDisconnect] = useState(false); // משתנה חדש כדי להבדיל בין ניתוק יוזם לבין ניתוק מרוחק
    ...
    const disconnect = () => {
        setIsInitiatingDisconnect(true); // סימון שהמשתמש יוזם את הניתוק
        openConfirmModal("Disconnect", "Are you sure you want to disconnect?").then((confirmed) => {
            ...
            setIsInitiatingDisconnect(false); // הסרת הסימון לאחר שהמשתמש בוחר להתנתק או לא
        });
    };
