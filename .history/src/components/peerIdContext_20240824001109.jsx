...
export const PeerIdProvider = ({ children }) => {
    ...
    const [initiatedDisconnect, setInitiatedDisconnect] = useState(false); // משתנה חדש לעקוב אחרי יוזמת ההתנתקות
            ...
            openConfirmModal("Incoming Connection", `Do you want to accept the connection request from ${con.peer}?`)
                .then((accepted) => {
                    if (accepted) {
                        ...
                        con.on("close", () => {
                            if (!initiatedDisconnect) { // אם המשתמש הנוכחי לא יזם את ההתנתקות
    ...
    const disconnect = () => {
        setInitiatedDisconnect(true); // סימון שהמשתמש הנוכחי יזם את ההתנתקות
    ...
    const handleRemoteDisconnect = () => {
        if (!initiatedDisconnect) { // רק אם המשתמש לא יזם את ההתנתקות
    ...
    return (
        <PeerIdContext.Provider
            value={{
                ...
                openConfirmModal,
                isDisconnected,
                initiatedDisconnect, // הוספת המשתנה לניהול ניתוקים
            }}
        >
            {children}
            {showConfirmModal && (
