...
export const PeerIdProvider = ({ children }) => {
    ...
    const [initiatedDisconnect, setInitiatedDisconnect] = useState(false); // משתנה חדש למעקב אחרי יוזמת ההתנתקות
            ...
            openConfirmModal("Incoming Connection", `Do you want to accept the connection request from ${con.peer}?`)
                .then((accepted) => {
                    if (accepted) {
                        ...
                        con.on("close", () => {
                            if (!initiatedDisconnect) { // רק אם הצד השני יזם את ההתנתקות
    ...
    const disconnect = () => {
        ...
        setInitiatedDisconnect(true); // יוזמת ההתנתקות מסומנת
        openConfirmModal("Disconnect", "Are you sure you want to disconnect?").then((confirmed) => {
            if (confirmed) {
                ...
                setInitiatedDisconnect(false); // אם המשתמש לא אישר את הניתוק, מבטלים את סימון ההתנתקות
            }
        });
    };
