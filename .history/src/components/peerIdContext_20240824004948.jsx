...
export const PeerIdProvider = ({ children }) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            con.on("close", () => {
                ...
                if (!initiatedDisconnect) {
                    ...
                    console.log("Disconnect was initiated by this user, no need to handle remote disconnect.");
                }
            });
        });
    };
    ...
    const disconnect = (initiatedByUser = false) => {
        ...
        // הודעה למשתמש השני לאחר ניתוק מלא
        if (initiatedByUser) {
            setTimeout(() => {
                if (peer) {
                    peer.disconnect();
                }
            }, 100); // המתנה קצרה לניתוק מלא לפני שליחת ההודעה למשתמש השני
        }
    ...
    const handleData = (data, senderPeerId) => {
        try {
            ...
            if (parsedData.type === "disconnect") {
                console.log("handleData - Received disconnect message");
                handleRemoteDisconnect(); // שליחה למשתמש השני להחלטה אם להתנתק
