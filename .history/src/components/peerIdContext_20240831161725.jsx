...
export const PeerIdProvider = ({ children }) => {
    ...
    // Initial Wallet and Peer creation
    useEffect(() => {
        const newWallet = ethers.Wallet.createRandom();
        const { publicKey } = newWallet;
        setMyWallet(newWallet);
    ...
    const connectToPeer = (recId) => {
        return new Promise((resolve, reject) => {
            if (!peer) {
                toast.error("Peer not initialized yet.");
                reject(new Error("Peer not initialized yet."));
                return;
            }
    ...
    const handleRemoteDisconnect = (notify = true, disconnectedPeerId = peerId) => {
        if (notify) {
            openConfirmModal(
                "Peer Disconnected",
