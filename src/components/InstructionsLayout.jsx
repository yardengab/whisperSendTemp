...
export default function InstructionsLayout({ title, steps }) {
    return (
        <div className="w-full bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl">
            ...
            {/* עדכון הגריד כך שהכל יהיה ממורכז במסכים גדולים */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center"> 
                {steps.map((step) => (
                    <div key={step.step} className="flex flex-col items-center text-center">
                        ...
                        <p className="font-bold text-sm mb-1 text-white">{step.title}</p> {/* טקסט בלבן */}
                        <p className="text-xs text-white">{step.description}</p> {/* טקסט בלבן */}
                    </div>
                ))}
            </div>
        </div>
