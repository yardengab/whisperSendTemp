...
import ToggleInstructionsButton from "../components/ToggleInstructionsButton"; // ייבוא הכפתור
...
export default function Decrypt() {
    ...
    const decryptionSteps = [
        { step: 1, color: 'blue', title: 'Upload an encrypted file', description: 'Choose an encrypted file to decrypt.' },
        { step: 2, color: 'green', title: 'Enter encryption key', description: 'Enter the encryption key or use your mnemonic.' },
        { step: 3, color: 'yellow', title: 'Decrypt', description: 'Click the "Decrypt" button to decrypt the file.' },
        { step: 4, color: 'red', title: 'Download decrypted file', description: 'Download the decrypted file once the process is complete.' },
    ];
    ...
    useEffect(() => {
        const handleResize = () => {
            setIsSmallScreen(window.innerWidth < 768); // להתאים את ההוראות למסכים קטנים
        };
    ...
    const handleDecrypt = async () => {
        ...
        try {
            ...
            reader.onload = () => {
                const encryptedJson = JSON.parse(Buffer.from(reader.result).toString());
                const decryptedBuffer = decryptFile(JSON.stringify(encryptedJson), wallet.publicKey, privateKey);
