...
import ToggleInstructionsButton from "../components/ToggleInstructionsButton"; // ייבוא הכפתור המשותף

const secureChatSteps = [
    { step: 1, color: 'blue', title: 'Send Invite', description: 'Send an invite to your peer to start a secure chat session.' },
    { step: 2, color: 'green', title: 'Connect', description: 'Enter the peer ID of your contact and connect securely.' },
    { step: 3, color: 'yellow', title: 'Chat Securely', description: 'Start chatting securely and send encrypted files.' },
    { step: 4, color: 'red', title: 'Recalculate Keys', description: 'Recalculate your encryption keys regularly for added security.' },
];

export default function ShareSecurely() {
    const { peerId, myWallet } = useContext(PeerIdContext); // נוספה גישה ל-myWallet
    const [isInviteOpen, setIsInviteOpen] = useState(false);
    const [showInstructions, setShowInstructions] = useState(false); 
    const [isSmallScreen, setIsSmallScreen] = useState(false); 
    const [showMnemonicPopup, setShowMnemonicPopup] = useState(false); // משתנה לפופ-אפ של המנמוניק
    const [isMnemonicConfirmed, setIsMnemonicConfirmed] = useState(false); // משתנה עבור אישור המנמוניק
    ...
    // בדיקה אם להציג את פופ-אפ המנמוניק
    useEffect(() => {
        const isMnemonicSaved = sessionStorage.getItem("mnemonicSaved");

        // הצגת הפופ-אפ רק אם המנמוניק לא נשמר
        if (!isMnemonicSaved) {
    ...
    const handleConfirmMnemonic = () => {
        if (isMnemonicConfirmed) {
            sessionStorage.setItem("mnemonicSaved", "true"); // שמירה שסטטוס נשמר ב-sessionStorage
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-gradient-to-r from-blue-500 to-indigo-500 dark:from-gray-800 dark:to-gray-900 relative">
            {/* הצגת ה-MnemonicPopup מעל התוכן */}
            {showMnemonicPopup && (
            ...
            <div className="w-full max-w-4xl">
                ...
                {/* ההוראות: במסכים גדולים יופיעו אוטומטית, במסכים קטנים רק בלחיצה */}
                {(showInstructions || !isSmallScreen) && (
                    <InstructionsLayout title="Secure Chat Instructions" steps={secureChatSteps} />
                )}
            </div>
